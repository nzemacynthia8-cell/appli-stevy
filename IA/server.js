...
app.listen(3000, () => {
  console.log("Serveur Stevy IA connecté à ChatGPT sur http://localhost:3000");
});
...
app.post("/create-image", async (req, res) => {
  ...
  try {
    // Exemple fictif : tu appelles une API d'image IA
    // Ici on renvoie une image de test (placeholder)
    const imageUrl = "https://via.placeholder.com/400x300.png?text=" + encodeURIComponent(prompt);
...
app.post("/create-image", async (req, res) => {
  ...
  try {
    // Pour tester sans API, on renvoie une image fictive (placeholder)
    const imageUrl = "https://via.placeholder.com/400x300.png?text=" + encodeURIComponent(prompt);
...
// ✅ Route POST "/create-image" connectée à DALL·E
app.post("/create-image", async (req, res) => {
  const { prompt } = req.body;
